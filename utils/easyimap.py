# -*- coding: utf-8 -*-
# Fork from https://github.com/keitaoouchi/easyimap/blob/master/easyimap/easyimap.py
...
class Imapper(object):
    def __init__(self, host, user, password, mailbox, timeout, ssl, port, **kwargs):
        self._fetch_message_parts = kwargs.get(
            "fetch_message_parts", "(UID RFC822)")
        self._read_only = kwargs.get("read_only", False)
        self._mailer = self._get_mailer(
            host, user, password, mailbox, timeout, ssl, port)
    ...
    def listup(self, limit=10, criterion=None, include_raw=False):
        ...
        result = []
        for i in email_ids:
            typ, content = self._mailer.uid(
                'fetch', i, self._fetch_message_parts)
            if typ == 'OK':
    ...
    def mail(self, uid, include_raw=False):
        ...
        typ, content = self._mailer.uid(
            'fetch', uid, self._fetch_message_parts)
        if typ == 'OK':
            mail = _parse_email(content, include_raw=include_raw)
            return mail
        else:
...
def _parse_email(data, include_raw=False):
    ...
    if not isinstance(string_or_bytes_uid, str):
        ...
        string_or_bytes_uid = string_or_bytes_uid.decode(
            encoding.get('encoding'))
